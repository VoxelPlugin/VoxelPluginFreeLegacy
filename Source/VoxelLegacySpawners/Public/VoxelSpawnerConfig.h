...
UCLASS(Abstract)
class UVoxelLegacySpawnerBase : public UObject
{
	GENERATED_BODY()

public:
	UPROPERTY(EditAnywhere, Category = "Voxel Legacy Spawner")
	mutable UObject* NewAsset = nullptr;
};
...
struct FVoxelSpawnerOutputName
{
	...
	template<typename T>
	FVoxelSpawnerOutputName(T Name)
		: Name(Name)
	{
	}
};
...
UCLASS()
class VOXELLEGACYSPAWNERS_API UVoxelSpawnerConfig : public UVoxelLegacySpawnerBase
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(EditAnywhere, Category = "Config")
	UVoxelGraphOutputsConfig* GeneratorOutputs;
