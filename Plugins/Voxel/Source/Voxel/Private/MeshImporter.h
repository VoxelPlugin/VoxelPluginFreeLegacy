...
void LineTrace(bool bAdd, UPrimitiveComponent* Component, TArray<std::forward_list<float>>& Values, FTransform& WorldTransform, FIntVector IMin, FIntVector Size, int X, int Y, int Z, int DeltaX, int DeltaY, int DeltaZ)
{
...
#endif // WITH_EDITOR
	float StartSign = (Component->GetDistanceToCollision(StartPosition, F) > 0) ? 1 : -1;
	float EndSign = (Component->GetDistanceToCollision(EndPosition, F) > 0) ? 1 : -1;
			...
			for (int Z = 0; Z < Size.Z; Z++)
			{
				for (int Sign = -1; Sign != 3; Sign += 2)
				{
					for (int i = 0; i < 3; i++)
					{
						...
						Delta[i] = Sign;
						LineTrace(bAdd, Component, Values, WorldTransform, IMin, Size, X, Y, Z, Delta[0], Delta[1], Delta[2]);
					}
				}
			}
		}
		...
		for (int Y = 0; Y < Size.Y; Y++)
		{
			for (int Z = 0; Z < Size.Z; Z++)
			{
				...
				if (!List.empty())
				{
					...
					UE_LOG(VoxelLog, Log, TEXT("%f"), Total / Count);
					World->SetValue(Position, Total / Count);
